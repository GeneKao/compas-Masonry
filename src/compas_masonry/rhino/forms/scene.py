...
from compas_masonry.rhino.objects import RhinoAssemblyObject

from compas_masonry.rhino.forms.assemblydata import AssemblyDataForm
from compas_masonry.rhino.forms.blockdata import BlockDataForm
    ...
    def OnCreateCell(self, args):
        def on_click(sender, e):
            ...
            if isinstance(data, Assembly):
                form = AssemblyDataForm(data)
            elif isinstance(data, Block):
                form = BlockDataForm(data)
            else:
...
class SceneObjectsForm(Eto.Forms.Dialog[bool]):
    def __init__(self, scene, title="Scene Objects", width=800, height=500):
        ...
        for obj in self.scene.objects:
            griditem = Eto.Forms.TreeGridItem(
                Values=(
                    str(obj.guid),
            ...
            collection.Add(griditem)
            if isinstance(obj, RhinoAssemblyObject):
                parent = griditem
                for block in obj.item.blocks():
                    griditem = Eto.Forms.TreeGridItem(
                        Values=('', '', '', '', '', block)
                    )
                    parent.Children.Add(griditem)
