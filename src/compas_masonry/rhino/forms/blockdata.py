...
from compas_ui.rhino.forms.meshdata import MeshDataForm


class BlockDataForm(MeshDataForm):
    """Form for working with block data.
    ...
    height : int, optional
        Window height.
    exclude_node_attr : list[str], optional
        The names of node attributes to exclude.
    ...
    def __init__(
        ...
        width=800,
        height=800,
        excluded_node_attr=None,
        excluded_edge_attr=None,
    ):

        self.assembly = assembly
        self.excluded_node_attr = excluded_node_attr
        ...
        self.nodepage = Page(
            title="Nodes",
            defaults=self.assembly.graph.default_node_attributes,
            keys=list(self.assembly.graph.nodes()),
            valuefunc=self.assembly.graph.node_attribute,
            excluded=self.excluded_node_attr,
        )
        self.edgepage = Page(
            title="Edges",
            defaults=self.assembly.graph.default_edge_attributes,
            keys=list(self.assembly.graph.edges()),
            valuefunc=self.assembly.graph.edge_attribute,
            excluded=self.excluded_edge_attr,
        )
        control = Eto.Forms.TabControl()
        control.TabPosition = Eto.Forms.DockPosition.Top
        control.Pages.Add(self.nodepage.widget)
...
class Page(object):
    ...
    keys : list[int | tuple[int, int]]
        The identifiers of the assembly components.
    valuefunc : callable
    ...
    rows : list[list[str]]
        The data per assembly components corresponding to the columns.
